...
def process():
    ...
    prof = pd.read_csv("alabama_proficiency_scores.csv")
    alabama_final_df = pd.merge(schools_data, prof, on='NAME', how='left')
    conn.close()

    # Create Plotly figure
    fig = px.scatter_mapbox(alabama_final_df, lat='LAT', lon='LON', zoom=5, mapbox_style='carto-positron', hover_name='NAME', hover_data='Average Assessment Proficiency', color='Average Assessment Proficiency')

    # Convert the Plotly figure to HTML
    plotly_html = fig.to_html(full_html=False)

    return render_template('result.html', schools_data=alabama_final_df, plotly_html=plotly_html, state_name=state_name)
